<?php  
  ...
  function getProfile()
  {
    ...
    if (mysqli_num_rows($results) == 1) { //user has been found in database
      $user=mysqli_fetch_array($results);
      $profileData = array("email"=> $user['email'], "firstname"=> $user['firstname'], "lastname"=> $user['lastname'],"username"=> $user['nickname'],"signature"=> $user['signature'], "birthdate"=> $user['birthdate'], "type"=> $user['image_type'], "image"=> $user['image'], "country"=> $user['country']); //complete with birthdate and country
      header("HTTP/1.1 200 OK");
